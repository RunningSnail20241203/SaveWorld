...
function generateItemPrompt(item, lineTheme) {
  ...
  const prompt = `游戏物品图标, 末世生存主题, ${item.name}, 等级${item.level}, 
...
function generateItemPromptSimple(item, lineTheme) {
  ...
  const prompt = `游戏图标, 末世生存, ${item.name}, Lv.${item.level}, ${item.description}, ${levelMod.color}, 透明背景, 80x80, Unity UI`;
...
const UI_PROMPT_TEMPLATES = {
  // 按钮
  button: `游戏UI按钮, 末世生存风格, 简洁设计, 
暖色调/冷色调, 圆角矩形, 适合点击, 
Unity UI Button使用, 128x48像素, 
无背景透明PNG格式, 悬停效果, 按下效果`,
