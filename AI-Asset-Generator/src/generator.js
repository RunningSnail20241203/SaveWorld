...
function getApiConfig() {
  ...
  try {
    _apiConfig = JSON.parse(fs.readFileSync(API_CONFIG_PATH, 'utf8'));
  } catch (e) {
    _apiConfig = {};
  }
  return _apiConfig;
...
/**
 * 根据元素类型获取推荐的生成尺寸
 * @param {string} type - 元素类型 (item/icon/button/panel/background/border/progressBar)
 * @returns {string} 尺寸字符串，如 "1024*1024"
 */
function getSizeForType(type) {
  const config = getApiConfig();
  const mapping = config.sizeMapping || {};
  return mapping[type] || mapping.default || config.defaultSize || '1024*1024';
}
...
async function generateSingleItemWithImage(itemName, imageOptions = {}) {
  const config = loadItemsConfig();
  const itemSize = imageOptions.size || getSizeForType('item');
  ...
  for (const [lineKey, lineData] of Object.entries(config.lines)) {
    ...
    if (item) {
      ...
      const imgResult = await generateImage(prompt, { ...imageOptions, size: itemSize, fileName: imgFileName, outputDir: IMAGES_ITEMS_DIR });
...
function collectAllItemTasks(imageOptions = {}) {
  ...
  for (const [lineKey, lineData] of Object.entries(config.lines)) {
    for (const item of lineData.items) {
      ...
      tasks.push({
        prompt,
        options: { ...imageOptions, size: itemSize, fileName: `${item.id}.png`, outputDir: IMAGES_ITEMS_DIR }
      });
    }
  }
...
function collectUITasks(imageOptions = {}) {
  ...
  for (const element of config.elements) {
    ...
    const filePath = path.join(UI_OUTPUT_DIR, fileName);
    savePrompt(filePath, prompt);
    const typeSize = imageOptions.size || getSizeForType(element.type);
    tasks.push({
      prompt,
      options: { ...imageOptions, size: typeSize, fileName: `${element.type}_${element.name}.png`, outputDir: IMAGES_UI_DIR }
    });
  }
